...
var contactKindEnum = pgEnum("contact_kind", ["customer", "vendor"]);
...
var accountTypeEnum = pgEnum("account_type", ["asset", "liability", "equity", "income", "expense"]);
var yesNoEnum = pgEnum("yes_no", ["yes", "no"]);
...
var journalSourceTypeEnum = pgEnum("journal_source_type", ["manual", "invoice", "expense", "payment"]);
...
var journalLines = pgTable("journalLines", {
  id: serial("id").primaryKey(),
...
var invoiceStatusEnum = pgEnum("invoice_status", ["draft", "sent", "paid", "overdue"]);
...
var invoiceLineItems = pgTable("invoiceLineItems", {
  id: serial("id").primaryKey(),
...
var billingFrequencyEnum = pgEnum("billing_frequency", ["one_time", "monthly"]);
var serviceCatalog = pgTable("serviceCatalog", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull(),
  name: varchar("name", { length: 180 }).notNull(),
...
var quoteStatusEnum = pgEnum("quote_status", ["draft", "sent", "accepted", "declined", "converted"]);
...
var quoteLineItems = pgTable("quoteLineItems", {
  id: serial("id").primaryKey(),
...
var expenseTags = pgTable("expenseTags", {
  id: serial("id").primaryKey(),
...
var journalEntryTags = pgTable("journalEntryTags", {
  id: serial("id").primaryKey(),
  journalEntryId: integer("journalEntryId").notNull(),
  tagId: integer("tagId").notNull()
}, (table) => [
...
var summaryCadenceEnum = pgEnum("summary_cadence", ["weekly", "monthly"]);
var summarySettings = pgTable("summarySettings", {
  id: serial("id").primaryKey(),
...
var summaryDeliveryStatusEnum = pgEnum("summary_delivery_status", ["sent", "failed"]);
var summaryDeliveries = pgTable("summaryDeliveries", {
  id: serial("id").primaryKey(),
...
async function ensureLedgerSetup(userId) {
  ...
  await db.insert(accounts).values(systemAccounts.map((account) => ({ ...account, userId, isSystem: "yes" }))).onConflictDoUpdate({
...
async function updateContact(userId, id3, input) {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable.");
  await db.update(contacts).set({ ...input, updatedAt: /* @__PURE__ */ new Date() }).where(and(eq(contacts.id, id3), eq(contacts.userId, userId)));
}
...
async function createExpense(input) {
  ...
  await db.update(expenses).set({ journalEntryId, updatedAt: /* @__PURE__ */ new Date() }).where(and(eq(expenses.id, expenseId), eq(expenses.userId, input.userId)));
}
...
async function archiveService(userId, serviceId) {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable.");
  await db.update(serviceCatalog).set({ isActive: "no", updatedAt: /* @__PURE__ */ new Date() }).where(and(eq(serviceCatalog.id, serviceId), eq(serviceCatalog.userId, userId)));
}
...
async function convertQuoteToInvoice(input) {
  ...
  await db.update(quotes).set({ status: "converted", convertedInvoiceId: invoiceId, updatedAt: /* @__PURE__ */ new Date() }).where(and(eq(quotes.id, quote.id), eq(quotes.userId, input.userId)));
...
async function sendInvoice(userId, invoiceId) {
  ...
  await db.update(invoices).set({ status: "sent", journalEntryId, updatedAt: /* @__PURE__ */ new Date() }).where(and(eq(invoices.id, invoice.id), eq(invoices.userId, userId)));
}
...
async function setInvoiceCheckoutSession(userId, invoiceId, sessionId) {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable.");
  await db.update(invoices).set({ stripeCheckoutSessionId: sessionId, updatedAt: /* @__PURE__ */ new Date() }).where(and(eq(invoices.id, invoiceId), eq(invoices.userId, userId)));
}
...
async function listEnabledSummarySettings() {
  const db = await getDb();
  if (!db) throw new Error("Database is unavailable.");
...
var resolveApiUrl = () => `${ENV.openRouterApiUrl.replace(/\/$/, "")}/chat/completions`;
var assertApiKey = () => {
...
function periodBoundsFor(cadence, now) {
  const periodEnd = cadence === "weekly" ? new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())) : new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));
...
async function financialSummaryHandler(req, res) {
  ...
  const results = [];
  try {
    const dueSettings = await listEnabledSummarySettings();
    for (const settings of dueSettings) {
      if (!isDueToday(settings, now)) {
        continue;
      }
      const { periodStart, periodEnd } = periodBoundsFor(settings.cadence, now);
      if (await hasSummaryDelivery(settings.id, periodStart, periodEnd)) {
      ...
      try {
        ...
        await recordSummaryDelivery({ settingsId: settings.id, periodStart, periodEnd, deliveryStatus: "failed", errorMessage: message });
...
function createApp() {
  const app2 = express();
  app2.post("/api/stripe/webhook", express.raw({ type: "application/json" }), stripeWebhookHandler);
  app2.post("/api/scheduled/financial-summary", financialSummaryHandler);
  app2.get("/api/scheduled/financial-summary", financialSummaryHandler);
